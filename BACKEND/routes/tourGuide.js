...
router.route("/update/:ID").put(async(req,res)=>{
    ...
    const update = await TourGuide.findByIdAndUpdate(tourGuideID, Update).then(()=>{
        res.status(200).send({status: "TourGuide detials updated"})
    }).catch((err)=>{
        console.log(err);
        res.status(500).send({status: "Error with updating event details", error: err.message});
    })
})
...
router.route("/delete/:id").delete(async(req,res)=>{
    ...
    await TourGuide.findByIdAndRemove(id).exec().then(()=>{
